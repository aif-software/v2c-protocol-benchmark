...
def run_benchmark(protocol, iterations):
    ...
    try:
        ...
        for i in range(iterations):
            ...
            oc_thread = threading.Thread(target=get_hardware_metrics, args=(f"{results_dir}/hardware_metrics_{i+1}.txt", 15, oc_stop), daemon=True)
            oc_thread.start()
            
            # start concurrent client
            
            subprocess.run([f"/home/ConnectiCar2.0/RaspberryPiScripts/venv/bin/python", f"{protocol}_main.py"])
        ...
        subprocess.run([
            ...
            results_dir,
            "--output", f"{results_dir}/{protocol}_test_{int(time.time())}_summary"
        ])
...
def get_hardware_metrics(out_path: str, interval_sec: int, stop_event: threading.Event):
    ...
    with open(out_path, "a", buffering=1) as f:
        while not stop_event.is_set():
            ...
            res = subprocess.run(["oc", "adm", "top", "pods"], capture_output=True, text=True)
            f.write(f"=== {ts} ===\n")
            if res.stdout:
                f.write(res.stdout)
            if res.stderr:
...
def get_network_metrics(duration, out_path="network_metrics.txt", ip="86.50.228.245", iperf_port="30685"):
    iperf_args = [
    "iperf3", "-c", ip, "-p", iperf_port, "-t", str(duration), "-i", "10"#, "-b", "2M"
    ]
    ...
    try:
        process = subprocess.Popen(iperf_args, stdout=iperf_out, stderr=subprocess.STDOUT)
        return process
...
def main():
    ...
    parser.add_argument("--protocol", choices=["mqtt", "amqp", "coap", "https"], required=True)
